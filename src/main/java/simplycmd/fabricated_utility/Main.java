	...
	public void onInitialize() {
		ServerPlayNetworking.registerGlobalReceiver(CYCLETRADES_PACKET_C2S.getIdentifier(), (server, player, handler, buffer, sender) -> {
			server.execute(() -> {
				...
				final List<VillagerEntity> list = player.getWorld().getEntitiesByClass(VillagerEntity.class, new Box(vec3d.getX() - 8.0, vec3d.getY() - 5.0, vec3d.getZ() - 8.0, vec3d.getX() + 8.0, vec3d.getY() + 5.0, vec3d.getZ() + 8.0), entity -> true);
				for (VillagerEntity entity : list) {
					if (entity.getCurrentCustomer() == player) {
						entity.setOffers(new TradeOfferList());
						entity.fillRecipes();
					}
				}
